// Package identity issues and verifies FlameGate API keys.
//
...
func New(keys *persistence.APIKeyRepo) *Service {
	return &Service{
		keys:      keys,
...
func (s *Service) Keys() *persistence.APIKeyRepo { return s.keys }
...
func (s *Service) Authenticate(ctx context.Context, plaintext string) (schema.APIKey, error) {
	if plaintext == "" {
		return schema.APIKey{}, ErrUnauthorized
	}
...
func (s *Service) cacheAuthResult(lookup string, rec schema.APIKey) {
	s.authMu.Lock()
...
func (s *Service) List(ctx context.Context, tenantID string) ([]schema.APIKey, error) {
	return s.keys.List(ctx, tenantID)
}

// Get returns a single key by id.
func (s *Service) Get(ctx context.Context, id string) (schema.APIKey, error) {
