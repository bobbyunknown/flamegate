...
import (
	...
	"github.com/bobbyunknown/flamegate/internal/shared/normalizer"
...
// chunk arrives within the timeout, the stream is canceled with ErrTimeout.
//
...
// tail before forwarding; on a Block decision the stream is canceled and an
...
func (p *Pipeline) pumpStream(ctx context.Context, req *core.ChatRequest, in <-chan core.StreamChunk, out chan<- core.StreamChunk,
	...
	if stallTimeout > 0 {
		...
		go func() {
			...
			if ctx.Err() == nil && stallCtx.Err() != nil {
				// Stall detected (parent ctx still alive but stall ctx canceled).
	...
	for {
		...
		case <-stallCtx.Done():
			if ctx.Err() != nil {
				// Parent context canceled (client disconnected).
				p.budgetRelease(scope)
				for range in {
				}
				return
...
func (p *Pipeline) planWithCooldownRetry(ctx context.Context, tenantID string, targets []dispatch.Target, required core.CapabilitySet, opts dispatch.PlanOptions) ([]dispatch.Attempt, error) {
	...
	if err == nil || !errors.Is(err, dispatch.ErrAllOnCooldown) {
		return attempts, err
	}
