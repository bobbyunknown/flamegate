...
// Chain ordering strategy tokens (persisted on schema.Chain.Strategy). "priority"
...
// orderStepsByCost is a no-op after model price table purge — keeps declared order.
func orderStepsByCost(steps []dispatch.Target) []dispatch.Target {
	return append([]dispatch.Target(nil), steps...)
}
...
type healthLatencyReader struct{ health *persistence.HealthRepo }
...
func (s *Handler) latencyReader() LatencyReader { return healthLatencyReader{health: s.health} }
