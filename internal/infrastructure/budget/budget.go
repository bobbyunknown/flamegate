...
func New(budgets *persistence.BudgetRepo, usage *persistence.UsageRepo) *Engine {
	return &Engine{
		budgets:      budgets,
		usage:        usage,
...
func (e *Engine) cachedListByScope(ctx context.Context, kind schema.BudgetScope, scopeID string) ([]schema.Budget, error) {
...
func (e *Engine) InvalidateBudgetCacheForScope(kind schema.BudgetScope, scopeID string) {
	key := string(kind) + ":" + scopeID
	e.budgetMu.Lock()
...
func (e *Engine) Check(ctx context.Context, scope Scope) (Decision, error) {
	...
	checks := []struct {
		kind schema.BudgetScope
		id   string
	}{
		{schema.ScopeAPIKey, scope.APIKeyID},
		{schema.ScopeProject, scope.ProjectID},
		{schema.ScopeTenant, scope.TenantID},
	}
	...
	var spendScopes []schema.SpendScope
	...
	for _, c := range checks {
		...
		for _, b := range budgets {
			...
			idx := len(spendScopes)
			spendScopes = append(spendScopes, schema.SpendScope{
				Kind:    schema.BudgetScope(b.ScopeKind),
				ScopeID: b.ScopeID,
				Since:   since,
			})
	...
	for _, ent := range entries {
		...
		reserved := e.getReserved(schema.BudgetScope(b.ScopeKind), b.ScopeID)
		spent += reserved
...
func (e *Engine) Reserve(ctx context.Context, scope Scope, estimatedMicros int64) error {
	...
	for _, bu := range dec.BudgetUsage {
		...
		key := reservationKey(schema.BudgetScope(bu.Budget.ScopeKind), bu.Budget.ScopeID)
...
func (e *Engine) releaseAll(scope Scope) {
	...
	for _, id := range []string{scope.APIKeyID, scope.ProjectID, scope.TenantID} {
		...
		delete(e.reservations, reservationKey(schema.ScopeAPIKey, id))
		delete(e.reservations, reservationKey(schema.ScopeProject, id))
		delete(e.reservations, reservationKey(schema.ScopeTenant, id))
	}
}

func reservationKey(kind schema.BudgetScope, scopeID string) string {
...
func (e *Engine) getReserved(kind schema.BudgetScope, scopeID string) int64 {
	e.mu.RLock()
	defer e.mu.RUnlock()
