...
func (m *Meter) recordUsage(ctx context.Context, rec schema.UsageRecord) error {
	if m.async != nil {
...
func NewAsyncWriter(usageStore UsageStore, cfg AsyncConfig) *AsyncWriter {
	...
	return &AsyncWriter{
		...
		log:             cfg.Logger,
		ch:              make(chan schema.UsageRecord, cfg.QueueSize),
...
func (w *AsyncWriter) Record(ctx context.Context, rec schema.UsageRecord) error {
	if w.closed.Load() {
...
func (w *AsyncWriter) run(ctx context.Context) {
	...
	batch := make([]schema.UsageRecord, 0, w.batchSize)
	flush := func() {
		if len(batch) == 0 {
			return
		}
