...
type ChatRequest struct {
	...
	// Headers carries safe-listed client HTTP headers for forwarding to
	// upstream providers. Router-internal; never serialized upstream.
	Headers map[string]string `json:"-"`
}

// ForwardedHeaders is the allowlist of client HTTP headers that may be
// forwarded to upstream providers via ChatRequest.Headers.
...
type RequestMetadata struct {
	...
	// APIKeyID is the id of the authenticated FlameGate key (for metering/audit).
	APIKeyID string
