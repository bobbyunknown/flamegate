...
export function CLIToolDetailPage() {
  ...
  const configureMut = useMutation({
    mutationFn: () =>
      api.cliToolConfigure(toolId!, {
        base_url: baseUrl,
        api_key: apiKey || "sk_flamegate",
        models: model ? [model] : undefined,
      }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cli-tools"] });
      toast.success("Tool configured", `${tool?.name} is now routing through FlameGate. All requests will use the proxy endpoint.`);
    },
  ...
  const removeMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["cli-tools"] });
      toast.success("Config removed", `${tool?.name} has been disconnected from FlameGate and will use its default endpoint.`);
    },
  ...
  if (!tool) {
    return (
      <div className="space-y-4">
        <Link to="/cli-tools" className="flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
  ...
  const snippetWithVars = tool.snippet
    ...
    .replace(/sk_flamegate/g, apiKey || "sk_flamegate");

  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <Link to="/cli-tools" className="mb-3 flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="h-4 w-4" /> Back to CLI Tools
        </Link>
            ...
            <div className="flex items-center gap-2">
              ...
              {keys.data?.keys && keys.data.keys.length > 0 ? (
                <Select
                  ...
                  className="flex-1"
                >
                  <option value="">sk_flamegate (default)</option>
