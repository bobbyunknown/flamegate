    ...
    <>
      ...
      <Card className="mb-6">
        ...
        <div className="px-6 pb-5 grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4">
          ...
          {/* Process column */}
          <div className="space-y-3">
            <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Process (PID {s.pid})</p>
            ...
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Open FDs</span>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 mb-6">
        <MetricChart
          title="Process CPU"
          description="flamegate's own CPU usage over time"
          icon={Cpu}
          data={chartData}
          dataKey="procCpu"
          color="#f59e0b"
        ...
        <MetricChart
          title="Process RSS"
          description="flamegate's resident memory over time"
          icon={MemoryStick}
          data={chartData}
          dataKey="procRss"
          color="#06b6d4"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border">
                    <th className="pb-2 pr-4 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">Time</th>
                    <th className="pb-2 pr-4 text-left text-xs font-medium uppercase tracking-wider text-muted-foreground">Type</th>
                    <th className="pb-2 pr-4 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground">CPU %</th>
                    <th className="pb-2 pr-4 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground">Mem %</th>
                    <th className="pb-2 text-right text-xs font-medium uppercase tracking-wider text-muted-foreground">Goroutines</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border">
                  {spikes.map((sp, i) => (
                    <tr key={i} className="group">
                      <td className="py-2 pr-4 font-mono text-xs text-muted-foreground">{tsLabel(sp.ts)}</td>
                      <td className="py-2 pr-4">
                        {sp.cpu_spike && <Badge variant="destructive">CPU spike</Badge>}
                        {sp.mem_spike && <Badge variant="destructive">Memory spike</Badge>}
                      </td>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 4, right: 8, left: -12, bottom: 0 }}>
              ...
              <Tooltip
                ...
                labelStyle={{ fontSize: 11, color: "var(--color-on-surface-variant)" }}
              ...
              <Area
                ...
                dot={false}
                activeDot={{ r: 4, stroke: color, strokeWidth: 2, fill: "var(--color-surface-container)" }}
              />
            </AreaChart>
          </ResponsiveContainer>
        )}
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-xs font-medium">
        ...
        <span className="tabular-nums text-foreground">{value.toFixed(1)}{unit}</span>
      </div>
      <Progress value={value} indicatorClassName={indicatorColor} className="h-2" />
