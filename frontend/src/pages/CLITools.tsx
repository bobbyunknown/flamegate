  ...
  return (
    <>
      <PageHeader
        title="CLI Tools"
        icon={TerminalSquare}
        description="One-click configuration for coding tools, wired to this FlameGate instance."
      />
      ...
      {tools.isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {(tools.data?.tools ?? []).map((t) => (
            <button
              key={t.id}
              onClick={() => navigate({ to: `/cli-tools/${t.id}` })}
              className="group flex items-center gap-3 rounded-xl border border-border bg-popover px-5 py-4 text-left transition-colors hover:border-accent-500/40 hover:shadow-md"
            >
              {/* Icon */}
              <ToolIcon id={t.id} />
