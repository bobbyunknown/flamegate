  ...
  return (
    <>
      ...
      <div className="mb-4 flex items-center gap-2.5">
        ...
        <span className="text-xs text-muted-foreground">API keys inherit rules from their assigned plan</span>
      </div>
      ...
      {plans.isLoading ? (
        ...
        <div className="rounded-2xl border-2 border-dashed border-border bg-background px-6 py-20 text-center">
          ...
          <h3 className="text-lg font-medium text-foreground">No plans configured</h3>
...
function OverviewStat({
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-border bg-popover px-4 py-3.5 shadow-sm">
...
function LimitStat({
  ...
  return (
    <div className="min-w-0">
      ...
      <dd className={`mt-0.5 truncate font-mono text-sm ${unlimited ? "text-muted-foreground" : "font-medium text-foreground"}`}>
        {unlimited ? "Unlimited" : value}
        {!unlimited && suffix && <span className="ml-1 font-sans text-xs font-normal text-muted-foreground">{suffix}</span>}
      </dd>
    </div>
  );
}
    ...
    <Card className="flex flex-col">
      ...
      <div className="flex items-start justify-between gap-3 px-5 pt-5 pb-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {p.hard_cutoff ? (
                <Badge variant="destructive" title="Requests are blocked once the budget is exhausted">hard cutoff</Badge>
              ) : (
                <Badge variant="secondary" title="Usage is tracked but requests are never blocked">advisory</Badge>
              )}
            </div>
            {p.description ? (
              <p className="mt-0.5 line-clamp-2 text-xs text-muted-foreground">{p.description}</p>
            ) : (
              <p className="mt-0.5 text-xs italic text-muted-foreground opacity-70">No description</p>
            )}
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-1">
          ...
          <Button variant="ghost" onClick={onDelete} className="px-2.5 py-2 text-muted-foreground hover:bg-[color:var(--color-danger)]/10 hover:text-[color:var(--color-danger)]" title="Delete plan">
        ...
        <div className="px-5 py-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {models.length > 0 ? (
              ...
              <span className="text-xs text-muted-foreground">All models allowed</span>
            )}
          </div>
        </div>
      </div>
      ...
      {/* Footer: keys + alert */}
      <div className="mt-auto flex items-center justify-between border-t border-border bg-muted px-5 py-3 text-xs">
        <div className="flex items-center gap-1.5 text-muted-foreground">
          <KeyRound className="h-3.5 w-3.5" />
          <span>
            <span className="font-medium text-foreground">{p.key_count}</span>{" "}
...
function PlanForm({ plan, onClose }: { plan?: Plan; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div className="px-5 py-4 space-y-6 overflow-y-auto min-h-0">
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Name this plan so you can identify it when assigning to keys.</p>
          </div>
          <Field label="Name">
            <Input
              value={name}
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Keys assigned to this plan inherit these limits.</p>
          </div>
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Control burst traffic per API key. Leave blank or 0 for unlimited.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3">
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Leave empty to allow all models. Keys can override this.</p>
          </div>
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">How to handle budget exhaustion.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-center justify-between gap-4 rounded-lg border border-border bg-muted px-3 py-2.5">
              <div className="min-w-0">
      ...
      <div className="shrink-0 flex gap-3 border-t border-border bg-muted px-6 py-4 rounded-b-xl">
