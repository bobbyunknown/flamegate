...
// FlameGate design system. Wrap the app in <ToastProvider> and call useToast()
...
> = {
  success: {
    icon: CheckCircle2,
    iconClass: "text-[color:var(--color-primary-container)]",
    bg: "bg-[color:var(--color-primary-container)]/10",
    border: "border-[color:var(--color-primary-container)]/30",
    progressClass: "bg-[color:var(--color-primary-container)]",
  },
  error: {
    icon: AlertCircle,
    ...
    progressClass: "bg-[color:var(--color-danger)]",
  },
  info: {
    icon: Info,
    ...
    >
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {toast.description && (
            <p className="mt-1 break-words text-[13px] leading-relaxed text-muted-foreground">{toast.description}</p>
          )}
        </div>
        <button
          onClick={onDismiss}
          className="-mr-1 -mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-black/5 hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-400/60 dark:hover:bg-white/10"
          aria-label="Dismiss"
        >
          <X className="h-4 w-4" />
        </button>
