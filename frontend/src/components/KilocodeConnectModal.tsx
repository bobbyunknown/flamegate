...
export function KilocodeConnectModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Modal open={true} onClose={onClose} title="Connect KiloCode" maxWidth="max-w-md">
      ...
      <div className="flex items-center justify-between rounded-xl border border-border bg-muted px-4 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Enter the code on the verification page</p>
          </div>
        </div>
...
export function Done({ provider }: { provider: string }) {
  return (
    <div className="flex flex-col items-center gap-3 py-6">
      ...
      <p className="text-xs text-muted-foreground">Refreshing {provider} accounts…</p>
    </div>
  );
}
