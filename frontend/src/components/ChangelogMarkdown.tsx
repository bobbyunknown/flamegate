...
export function ChangelogMarkdown({
  ...
  return (
    <div className={compact ? "space-y-3 text-foreground" : "space-y-5"}>
      {releases.map((rel, releaseIndex) => (
        <div key={rel.version} className={compact && releaseIndex > 0 ? "border-t border-border pt-3" : undefined}>
          {/* Version header */}
          <div className="mb-2 flex min-w-0 items-center gap-2">
            ...
            {rel.date && (
              <span className="min-w-0 truncate text-xs text-muted-foreground">{rel.date}</span>
            )}
          </div>
                  ...
                  <div key={sec.category}>
                    ...
                    <ul className={compact ? "space-y-1" : "space-y-1"}>
                      {sec.entries.map((entry, i) => (
                        <li
                          key={i}
                          className="flex min-w-0 items-start gap-1.5 text-xs leading-relaxed text-foreground"
                        >
                          <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-muted-foreground" />
                          <span className="min-w-0 break-words">
                            {entry.scope && (
                              <span className="mr-1 inline-flex items-center rounded bg-muted px-1 py-px font-mono text-[10px] font-medium text-muted-foreground">
                                {entry.scope}
                              </span>
                            )}
                            {entry.description}
