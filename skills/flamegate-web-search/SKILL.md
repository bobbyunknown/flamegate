---
name: flamegate-web-search
description: Web search via FlameGate /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# FlameGate — Web Search
...
```bash
curl $FLAMEGATE_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$FLAMEGATE_URL/v1/models/info?id=tavily/search"
```
...
```bash
curl -X POST $FLAMEGATE_URL/v1/search \
  -H "Authorization: Bearer $FLAMEGATE_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily","query":"FlameGate open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.FLAMEGATE_URL}/v1/search`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.FLAMEGATE_KEY}`, "Content-Type": "application/json" },
  body: JSON.stringify({ model: "tavily", query: "latest LLM benchmarks", max_results: 10 }),
});
