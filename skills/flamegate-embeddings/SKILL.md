---
name: flamegate-embeddings
description: Generate vector embeddings via FlameGate /v1/embeddings using OpenAI / Gemini / Mistral / Voyage / Nvidia embedding models for RAG, semantic search, similarity. Use when the user wants embeddings, vectors, RAG, semantic search, or to embed text.
---

# FlameGate — Embeddings
...
```bash
curl $FLAMEGATE_URL/v1/models/embedding | jq '.data[].id'
# Per-model dimensions
curl "$FLAMEGATE_URL/v1/models/info?id=openai/text-embedding-3-small"
```

## Endpoint

`POST $FLAMEGATE_URL/v1/embeddings`
...
```bash
curl -X POST $FLAMEGATE_URL/v1/embeddings \
...
```js
const r = await fetch(`${process.env.FLAMEGATE_URL}/v1/embeddings`, {
  method: "POST",
